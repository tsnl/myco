...
pub fn text_parts(result: &ToolResult) -> Vec<String> {
    result
        .content
        .iter()
...
/// [`myco_session::lock_myco_home_for_test`] so mutating tests serialize.
...
pub fn temp_home(tag: &str) -> TempHome {
    let lock = myco_session::lock_myco_home_for_test();
    let dir = std::env::temp_dir().join(format!("myco-{tag}-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    // SAFETY: test-only env override; held under the myco-home lock.
    unsafe {
