//! Minimal pty plumbing: `openpty`, a child wired to the slave as its
//! controlling terminal, and nonblocking async master I/O.
//!
//! Deliberately libc-only (no pty crate): the whole need is one `openpty`,
//! two fcntls, and read/write through [`tokio::io::unix::AsyncFd`].
//! Ported verbatim from v2 (`main-v2:src/machines/tool_services/bash_service/pty.rs`).
...
/// controlling terminal. For use in `pre_exec` — `setsid` puts the child in
/// a fresh process group of its own pid, so `kill(-pid)` keeps working.
///
/// # Safety
