...
//! agent run that writes the summary is [`myco_agent::run_compact_worker`].
...
/// Serialize tests that mutate `MYCO_HOME` (process-global env). Exposed
/// unconditionally so `myco-test-support` (a dev-dependency of other crates)
/// can reach it; not part of the runtime API.
#[doc(hidden)]
pub fn lock_myco_home_for_test() -> std::sync::MutexGuard<'static, ()> {
...
mod tests {
    ...
    fn corrupt_and_wrong_version_files_are_reported_as_skipped() {
        let root = myco_test_support::temp_dir("session-skip");
        let dir = root.path();
