...
//! ([`myco_session::compact_session`], [`myco_session::link_compact_pair`])
...
mod tests {
    ...
    fn a_stale_summary_is_rejected_instead_of_reused() {
        let tmp = myco_test_support::temp_dir("compact-stale");
        let dir = tmp.path();
    ...
    fn missing_or_empty_summary_fails_loudly() {
        let tmp = myco_test_support::temp_dir("compact-missing");
        let dir = tmp.path();
        let path = dir.join("s.summary.md");
