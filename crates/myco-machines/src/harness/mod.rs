...
use myco_core::{Async, CancelToken};
use myco_models as generative_model;

pub use myco_config::harness::{
...
mod tests {
    ...
    fn standard_catalog_is_bash_editor_view_image_only() {
        let names: Vec<_> = crate::host::HostWorker::standard_tool_specs(
            myco_config::DEFAULT_MAX_IMAGE_BASE64_BYTES,
        )
        .into_iter()
        .map(|s| s.name)
        .collect();
        ...
        // The manual is files on disk (`myco_prompts::manual::export`), not a tool.
    ...
    async fn root_only_tools_keep_host_field_and_run_local() {
        ...
        let _home = myco_test_support::temp_home("root-only-host");
