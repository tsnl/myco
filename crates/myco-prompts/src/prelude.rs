//! The agent prelude: maildir-style entries under `~/.myco/v2/workspace/prelude/`.
//!
...
//! each agent system prompt ([`crate`]); the root-only `prelude` tool
//! (`PreludeTool` in `myco-machines`) is the edit path. The storage rules
...
pub fn dir() -> Result<PathBuf, String> {
    Ok(myco_core::data_root()?.join("workspace").join("prelude"))
}
...
/// (`myco_machines::harness::StartupPreflight`).
...
pub fn add_entry(
    ...
    for _ in 0..8 {
        let name = format!(
            ...
            &myco_core::uuid_simple_hex(uuid::Uuid::new_v4())[..4]
        );
