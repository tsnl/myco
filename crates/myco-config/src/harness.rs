...
//! `max_soul_bytes`) come from `~/.myco/config.toml`
//! ([`crate::file`]), resolved in [`crate::Config`].

use std::collections::HashSet;
use std::path::{Path, PathBuf};

/// One host in the pool: a name and the argv that spawns its worker.
...
pub struct HarnessConfig {
    /// Remote hosts only. Local is always added in-process by the harness.
...
/// Local-only pool with stand-in knobs — **test fixture only**. Every real run
/// builds its pool through [`HarnessConfig::from_ssh_aliases`] with values
/// resolved by [`crate::Config`], which is where the actual defaults live.
...
impl HarnessConfig {
    ...
    /// resolved connect timeout and image cap ([`crate::Config`]
...
mod tests {
    ...
    fn harness_from(ssh_config: &str, attach_timeout_secs: u64) -> HarnessConfig {
        HarnessConfig::from_ssh_aliases(
            ...
            attach_timeout_secs,
            crate::DEFAULT_MAX_IMAGE_BASE64_BYTES,
        )
    }
...
"#;
    ...
    fn include_directives_are_followed() {
        let dir = myco_test_support::temp_dir("sshconf-include");
    ...
    fn include_subfile_with_quoted_path_and_eq_form() {
        let dir = myco_test_support::temp_dir("sshconf-sub");
    ...
    fn include_cycle_terminates() {
        let dir = myco_test_support::temp_dir("sshconf-cycle");
