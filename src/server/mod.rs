...
//! it is a second front-end, not a second runtime.
...
use crate::generative_model::Effort;
...
pub async fn serve(harness: Arc<Harness>, config: ServerConfig) -> Result<(), String> {
    let ServerConfig {
        bind,
        port,
        default_effort,
    ...
    let state = AppState::new(harness, default_effort, debug_dump_api_requests, dist_dir);
