...
//!   (the non-TTY / [`Palette::plain`] guarantee for pipes, session files,
//!   and echoed input) — no delimiter is dropped.
//! - **Styled = presentation**: formatting delimiters are consumed into SGR /
//!   OSC 8 rather than printed, while every *content* byte still reaches the
//!   output in order (ATX headers keep their visible `#`). A stray delimiter
//!   can mis-style a span, never corrupt content.
//!
