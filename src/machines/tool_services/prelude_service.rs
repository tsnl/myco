//! Root-only tool: edit the agent prelude (`~/.myco/v2/workspace/prelude/` entries).
//!
...
//! Storage semantics live in [`crate::prompts::prelude`]; this service adds the
//! model surface and reports the rendered size against `max_prelude_bytes` so
//! agents notice cap pressure while curating, not at the next startup warning.
...
Edit your prelude: the maildir-style entries under `~/.myco/v2/workspace/prelude/` rendered, every
...
mod tests {
    ...
    async fn call(tool: &Arc<PreludeTool>, input: serde_json::Value) -> ToolResult {
        tool.clone()
            .dispatch_tool_use(
                ToolUse {
                    id: "t1".into(),
