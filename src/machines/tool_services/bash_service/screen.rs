//! Rendering the vt100 screen model to [`ShellScreen`]: styled cell runs
//! with the xterm-256 palette resolved to concrete colors, the cursor as
//! its own run — one definition of "what the terminal shows" serving the
//! agent's `screenshot`, the HTTP wire, and the WebSocket pusher alike.
...
pub(super) fn render_screen(shared: &SessionShared) -> ShellScreen {
