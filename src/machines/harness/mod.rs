...
use crate::machines::tool_services::ToolService;
use crate::models as generative_model;

pub use crate::config::harness::{
    HarnessConfig, default_ssh_config_path, load_ssh_host_aliases, ssh_config_host_aliases,
    ssh_spawn_command,
};

// HostController lives in `crate::machines::host` (in-process local or remote subprocess).
pub use crate::machines::host::{HostConfig, HostController};
...
impl Harness {
    ...
    pub async fn attach_with_root_services(
        ...
        let mut harness = Self::local_harness(root_services, config.max_image_base64_bytes);
    ...
    pub fn local_with_services(extra: Vec<Arc<dyn ToolService>>) -> Arc<Self> {
        Arc::new(Self::local_harness(
            extra,
            HarnessConfig::default().max_image_base64_bytes,
        ))
    }
    ...
    fn local_harness(
        ...
        for spec in crate::machines::host::HostWorker::standard_tool_specs(max_image_base64_bytes) {
        ...
        let local_bash = Arc::new(crate::machines::tool_services::BashService::new());
        let mut local_services: Vec<Arc<dyn ToolService>> =
            crate::machines::host::HostWorker::standard_services_with(
                local_bash.clone(),
                max_image_base64_bytes,
            );
        local_services.extend(root_services);

        let local_worker = Arc::new(
            crate::machines::host::HostWorker::new("local", local_services).with_bash(local_bash),
        );
    ...
    pub async fn shell_overviews(
        &self,
    ) -> Vec<(String, crate::machines::tool_services::ShellOverview)> {
        let mut out = Vec::new();
    ...
    pub fn dispatch_tool_use(
        self: Arc<Self>,
        mut tool_use: myco_types::ToolUse,
        agent_id: uuid::Uuid,
        cancel: CancelToken,
        ...
        Box::pin(async move {
            ...
            if !self.host_tool_names.contains(&name) {
                return myco_types::ToolResult::err(format!("unknown tool '{name}'")).with_id(id);
            }
            ...
            let host_name = if self.root_only_tool_names.contains(&name) {
                ...
                match resolve_host_for_call(&tool_use, &self.default_host) {
                    Ok(h) => h,
                    Err(e) => return myco_types::ToolResult::err(e).with_id(id),
                }
            };
...
fn strip_host_field(tool_use: &mut myco_types::ToolUse) {
...
mod tests {
    ...
    fn standard_catalog_is_bash_editor_view_image_only() {
        let names: Vec<_> = crate::machines::host::HostWorker::standard_tool_specs(
        ...
        // The manual is files on disk (`crate::prompts::manual::export`), not a tool.
    ...
    fn standard_tools_do_not_declare_their_own_host_property() {
        for spec in crate::machines::host::HostWorker::standard_tool_specs(
            crate::config::DEFAULT_MAX_IMAGE_BASE64_BYTES,
        ) {
    ...
    async fn root_only_tools_keep_their_schema_and_run_local() {
        use crate::machines::tool_services::SessionMetaTool;
        use crate::session::{ActiveSession, Session};

        let _home = crate::test_support::temp_home("root-only-host");
    ...
    async fn remote_shells_are_listed_and_driven_over_the_protocol() {
        ...
        let (previous, overview) = rem
            .shell_lock("rterm", crate::machines::tool_services::ShellLock::User)
            .await
            .expect("lock");
        assert_eq!(
            previous,
            crate::machines::tool_services::ShellLock::Assistant
        );
        assert_eq!(
            overview.lock,
            crate::machines::tool_services::ShellLock::User
        );
