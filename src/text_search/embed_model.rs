...
//! tokenizer into `embed_weights/` (gitignored). Runtime never hits HF or ORT.
...
static MODEL_WEIGHTS: &[u8] = include_bytes!("embed_weights/model.safetensors");
static TOKENIZER_JSON: &[u8] = include_bytes!("embed_weights/tokenizer.json");
static CONFIG_JSON: &[u8] = include_bytes!("embed_weights/config.json");
    ...
    let slot = EMBEDDER.get_or_init(|| match load_minilm() {
        ...
        Err(e) => Err(format!(
            "MiniLM (Candle) embedder init failed: {e}. \
             Assets come from src/text_search/embed_weights/ (build.rs + include_bytes). \
             Rebuild with network + curl, or pre-seed that directory. \
             See embed_weights/README.md and `myco --help harness-ops`."
        )),
    });
    match slot {
        Ok(m) => m
