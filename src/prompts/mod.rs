...
pub fn agent_prompt_epilogue() -> String {
    epilogue_with(crate::core::data_root().ok(), std::env::current_dir().ok())
}
...
pub fn prelude_oversize(max_prelude_bytes: usize) -> Option<PreludeOversize> {
    oversize_at(&crate::prompts::prelude::dir().ok()?, max_prelude_bytes)
}

/// [`prelude_oversize`] against an explicit directory.
fn oversize_at(dir: &std::path::Path, max_bytes: usize) -> Option<PreludeOversize> {
    let entries = crate::prompts::prelude::entries(dir);
    let bytes = crate::prompts::prelude::rendered_body(&entries).len();
...
fn rendered_prelude(dir: &std::path::Path) -> Option<String> {
    let entries = crate::prompts::prelude::entries(dir);
    (!entries.is_empty()).then(|| crate::prompts::prelude::rendered_body(&entries))
}
...
mod tests {
    ...
    fn epilogue_includes_always_on_policy() {
        for needle in [
            ...
            // Machines outside the declared inventory need the user's say-so.
