...
- Updating `myco` on **remote** hosts: compile **on the target** (see `manual` `harness-ops`).
  ...
  build (`build.rs` curls safetensors into `src/text_search/embed_weights/`,
  gitignored — weights are **not** in git archive / GitHub source tarballs). Never scp prebuilt
