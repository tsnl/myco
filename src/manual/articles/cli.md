...
until a provider usage report arrives. Below the token counts, one `↻`-prefixed line per
still-running tool (live bash session on the in-process local host) shows its command, uptime,
and idle time; remote hosts are not queried for this.
