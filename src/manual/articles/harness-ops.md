...
1. **Host down / unavailable**
   ...
   - Startup checks expected executables on the **agent** machine (`bash`,
     `lynx`, `kill`; `ssh`/`ssh-add`/`ssh-keygen` when remotes are configured)
     and reports missing ones in the startup WARNING block — the user must
     install them and restart myco. Remote hosts report missing programs as
     tool errors at call time.
