...
All instances currently use the server's launch directory for local tools and
served files. A profile's `workspace/` is its persistent notes/prelude directory,
not the tools' working directory. Profiles separate application state; they are
not operating-system sandboxes or access-control boundaries.
...
API-created sessions use the server's launch directory and addressed profile; no new
