...
/// removal must not survive the roster reconciliation re-adding the name.
#[test]
...
/// An `auth.json` written by the password era (with `password_hash` fields)
/// still opens: the fields are simply ignored, and the next write drops them.
#[test]
fn a_password_era_snapshot_still_opens() {
    let dir = crate::test_support::temp_dir("auth-legacy");
    let path = dir.path().join("auth.json");
    std::fs::write(
        &path,
        r#"{"users": [{
