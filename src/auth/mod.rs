...
//!   one (from the admin panel, or the server prints one at startup when
//!   nobody could otherwise sign in); it redeems exactly once at the token
//!   endpoint (`grant_type=code`, the shape of an RFC 6749 extension grant)
//!   for a bearer token with a fixed lifetime.
//! - **Passkeys**, the steady state: WebAuthn credentials enrolled by any
//!   signed-in session, persisted in `passkeys.json`.
//!
//! ## Where the data lives
//!
//! Keyspaces in memory, with deliberately different durability:
//!
//! - **users** — id → [`StoredUser`] (name, disabled flag, timestamps).
//!   Snapshotted to `$MYCO_HOME/v2/auth.json` on every write, so a disabled
//!   account stays disabled across restarts.
//! - **passkeys** — id → enrolled credentials, in `passkeys.json`. The only
//!   durable secret-holder, and it holds public keys.
//! - **tokens, codes, ceremonies** — never persisted. A restart logs everyone
//!   out and voids every outstanding code, which is the behavior you want
//!   from a process that just changed underneath its clients.
//!
