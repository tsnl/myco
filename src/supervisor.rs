//! The session runtime core: frontend-neutral. The web server (`server`) and
//! the CLI (`cli`) are both thin adapters over this — one supervisor, live
//! sessions with queued commands, per-session event feeds, compaction, and
//! the `subagent` tool all live here.
...
impl Supervisor {
    ...
    pub async fn retire(&self, id: &str) -> Option<Arc<Live>> {
