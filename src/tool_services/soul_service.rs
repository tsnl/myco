//! Root-only tool: edit the agent soul (`~/.myco/workspace/soul/` entries).
//!
//! Installed on the in-process local worker only — the soul lives on the
//! user's machine (brains stay local), so there is nothing to run remotely.
//! Storage semantics live in [`crate::soul`]; this service adds the model
//! surface and reports the rendered size against `max_soul_bytes` so agents
...
Edit your soul: the maildir-style entries under `~/.myco/workspace/soul/` rendered, every
entry in filename order, into the `# Soul` section of every agent system prompt.

The soul is the default home for durable information — record findings eagerly (user
...
See "Workspace & soul" in your prompt. Entries are write-once files; use this tool, not
...
- list: the live entries with ids and text, plus total rendered size against
  `max_soul_bytes`. The `# Soul` in your prompt is a snapshot from model build time;
...
/// Local tool over the soul directory; carries the resolved `max_soul_bytes`
...
impl SoulTool {
    pub fn new(max_soul_bytes: usize) -> Self {
        Self { max_soul_bytes }
    }
}

impl ToolService for SoulTool {
    ...
    fn dispatch_tool_use(
        ...
        Box::pin(async move {
            let input: Input = match serde_json::from_value(tool_use.input.clone()) {
                Ok(v) => v,
                Err(e) => return ToolResult::err(format!("invalid soul input: {e}")),
            };
            ...
            match tokio::task::spawn_blocking(move || self.execute(input)).await {
                ...
                Err(e) => ToolResult::err(format!("soul join: {e}")),
            }
        })
    }
}
...
impl SoulTool {
    fn execute(&self, input: Input) -> Result<String, String> {
        ...
        match input.action {
            ActionKind::Add => {
                ...
                let name = soul::add_entry(&dir, &text)?;
                Ok(format!("added soul/{name}\n{}", self.status(&dir)))
            }
            ActionKind::Replace => {
                let id = required_id(input.id, "replace")?;
                let text = required_text(input.text, "replace")?;
                if !soul::is_entry_name(&id) {
                    return Err(format!(
                        "{id:?} is not a soul entry id (a plain `*.md` filename, as listed)"
                    ));
                }
                // Replacement lands before the old entry goes, so a crash or
                // race between the two steps duplicates — never loses.
                let name = soul::add_entry(&dir, &text)?;
                let note = match soul::remove_entry(&dir, &id)? {
                    true => String::new(),
                    false => format!(
                        "note: soul/{id} was already gone (concurrent edit?) — \
                         action=list and merge any duplicate\n"
                    ),
                };
                Ok(format!(
                    "replaced soul/{id} with soul/{name}\n{note}{}",
                    self.status(&dir)
                ))
            }
            ActionKind::Remove => {
                let id = required_id(input.id, "remove")?;
                match soul::remove_entry(&dir, &id)? {
                    true => Ok(format!("removed soul/{id}\n{}", self.status(&dir))),
                    false => Err(format!(
                        "soul/{id} not found — already removed or replaced by another \
                         agent; action=list shows the live entries"
                    )),
                }
            }
            ActionKind::List => {
                let entries = soul::entries(&dir);
                if entries.is_empty() {
                    return Ok(format!("(no soul entries)\n{}", self.status(&dir)));
                }
                Ok(format!(
                    "{}\n\n{}",
                    self.status(&dir),
                    soul::rendered_body(&entries)
                ))
            }
        }
    }
    ...
    /// soul no longer fits — the agent holding the pen is the one who can
    /// merge entries, so it hears about cap pressure immediately.
    fn status(&self, dir: &Path) -> String {
        let entries = soul::entries(dir);
        let bytes = soul::rendered_body(&entries).len();
        let mut out = format!(
            "soul: {} entr{}, {bytes} bytes rendered of {} max_soul_bytes\n",
            entries.len(),
            if entries.len() == 1 { "y" } else { "ies" },
            self.max_soul_bytes,
        );
        if bytes > self.max_soul_bytes {
            out.push_str(
                "WARNING: over max_soul_bytes — future prompts truncate the tail; merge \
...
fn required_id(id: Option<String>, action: &str) -> Result<String, String> {
    match id {
        ...
        _ => Err(format!(
            "{action} requires `id` (the entry filename, as listed in `# Soul` / action=list)"
        )),
    }
}
...
mod tests {
    ...
    async fn call(tool: &Arc<SoulTool>, input: serde_json::Value) -> generative_model::ToolResult {
        tool.clone()
            .dispatch_tool_use(
    ...
    fn input_schema_is_flat_required_and_closed() {
        let spec = SoulTool::new(1024).tool_specs().remove(0);
    ...
    async fn add_replace_remove_list_round_trip() {
        let _home = temp_home("soul-tool");

        let tool = Arc::new(SoulTool::new(1024 * 1024));
        ...
        assert!(added_text.contains("added soul/"), "{added_text}");
        assert!(added_text.contains("soul: 1 entry"), "{added_text}");
        let id = added_text
            .lines()
            .next()
            .unwrap()
            .strip_prefix("added soul/")
            .unwrap()
            .to_string();
        assert!(soul::is_entry_name(&id), "{id}");
        ...
        assert!(
            result_text(&listed).contains(&format!("[soul entry {id}]\nfact one")),
            "{listed:?}"
        );
        ...
        assert!(
            replaced_text.contains(&format!("replaced soul/{id} with soul/")),
            "{replaced_text}"
        );
        assert!(replaced_text.contains("soul: 1 entry"), "{replaced_text}");
        let listed = result_text(&call(&tool, serde_json::json!({"action": "list"})).await);
        assert!(listed.contains("fact one, amended"), "{listed}");
        assert!(!listed.contains(&format!("[soul entry {id}]")), "{listed}");
        ...
        assert!(listed.contains("soul: 2 entries"), "{listed}");

        // remove deletes by id; removing again is a loud error, not a wipe.
        let live_id = listed
            .lines()
            .find_map(|l| {
                l.strip_prefix("[soul entry ")
    ...
    async fn guards_and_cap_warning() {
        let _home = temp_home("soul-tool-guards");

        let tool = Arc::new(SoulTool::new(64));
        ...
        assert!(
            result_text(&big).contains("WARNING: over max_soul_bytes"),
            "{big:?}"
        );
    }
}
