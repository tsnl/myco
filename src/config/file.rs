...
//! and scalar knobs (`attach_timeout_secs`, `max_memory_bytes`). Scalar knobs parse as `Option`
...
pub struct FileConfig {
    ...
    /// Cap on the rendered memory (`workspace/memory/` entries) appended to every
    /// agent system prompt. A longer memory is cut to this many bytes with a
    /// marker, and startup warns loudly that it happened. `None` = unset;
    /// [`crate::prompts::DEFAULT_MAX_MEMORY_BYTES`] applies at resolve.
    #[serde(default)]
...
mod tests {
    ...
    fn scalar_knobs_parse_as_set_or_unset() {
        ...
        assert_eq!(parse_file_config_str("").unwrap().max_memory_bytes, None);
        assert_eq!(
            parse_file_config_str("max_memory_bytes = 4096")
                .unwrap()
                .max_memory_bytes,
            Some(4096)
        );
