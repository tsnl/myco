...
pub struct Config {
    ...
    pub model: String,
    /// Cap on the prelude appended to every agent system prompt (config file
    /// `max_prelude_bytes`, default [`crate::prompts::DEFAULT_MAX_PRELUDE_BYTES`]).
...
/// Overlay a [`RetryEntry`] onto [`RetryPolicy::default`] field by field, so
...
"#;
    ...
    /// Retry follows `auth`'s override rule — the model's table replaces the
    /// gateway's outright — while each *unset field* falls back to the built-in
    /// default rather than to the gateway.
    #[test]
...
"#;
        ...
        let default = crate::generative_model::RetryPolicy::default();
...
"#;
    ...
    fn max_prelude_bytes_comes_from_the_config_file() {
        ...
        // 256 KiB backstop.
        assert_eq!(resolve(""), crate::prompts::DEFAULT_MAX_PRELUDE_BYTES);
